...
export default function Home() {
  ...
  const handleBetaRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Track event in PostHog (if analytics context is available)
      // analytics.trackEvent("beta_access_requested", { email })
    } catch (error: any) {
              ...
              <div className="mx-auto max-w-3xl text-center space-y-6">
                ...
                <div className="pt-6">
                  {!showForm ? (
                    ...
                    <form onSubmit={handleBetaRequest} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                      <div>
                      ...
                      <div className="flex gap-3">
                        <button
                          type="submit"
                          disabled={loading}
                          className="flex-1 rounded-full bg-gradient-to-r from-teal-500 to-cyan-500 px-6 py-2.5 text-base font-semibold text-white shadow-lg shadow-teal-500/25 transition-all hover:scale-105 hover:shadow-xl hover:shadow-teal-500/30 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                          ...
                          disabled={loading}
                          className="rounded-full border border-white/20 bg-white/5 px-6 py-2.5 text-base font-semibold text-white backdrop-blur-sm transition-colors hover:border-white/30 hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          Cancel
                        </button>
                      </div>
